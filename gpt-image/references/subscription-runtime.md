...
- The runner can start several independent `codex exec` turns through `batch`. It checks ChatGPT auth once, then uses bounded concurrency (2 by default, 4 maximum). This is local CLI orchestration, not an atomic OpenAI batch-image feature.
